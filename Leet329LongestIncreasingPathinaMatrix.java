...
public class Leet329LongestIncreasingPathinaMatrix {
	 ...
	 * Compare 4 direction and skip cells that are out of boundary or smaller
	...
	public int longestIncreasingPath(int[][] matrix) {
		...
		int[][] cash = new int[m][n];
		// at least 1
		int max = 1;
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				int len = dfs(matrix, i, j, m, n, cash);
	...
	public int dfs(int[][] matrix, int i, int j, int m, int n, int[][] cash) {
		...
		for (int[] dir : dirs) {
			...
			if (x >= 0 && x < m && y >= 0 && y < n && matrix[x][y] > matrix[i][j]) {
				int len = 1 + dfs(matrix, x, y, m, n, cash);
				max = Math.max(len, max);
			}
		}
		// update it
